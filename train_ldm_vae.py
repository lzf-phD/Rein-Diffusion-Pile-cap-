...
def train(args):
    ...
    im_dataset = ReinDataset(split='train',
                              im_path=dataset_config['im_path'],
                              im_h=dataset_config['im_h'],
                              im_w=dataset_config['im_w'],
                              im_channels=dataset_config['im_channels'],
                              use_latents=True,
                              latent_path=os.path.join(train_config['task_name'],
                                                       train_config['vae_latent_dir_name']) #vae_latents
                              )
    ...
    for epoch_idx in range(num_epochs):
        losses = []
        for im,txt in tqdm(data_loader):
            optimizer.zero_grad()
            im = im.float().to(device)
            txt_embedding = txt.float().to(device)
            if im_dataset.use_latents: #true
            ...
            noise_pred = model(noisy_im,txt_embedding,t)
        ...
        print('Finished epoch:{} | Loss : {:.4f}'.format(
            ...
            np.mean(losses)))
        lr_scheduler.step()
        if epoch_idx % 10 == 0 or epoch_idx + 1 == num_epochs:
            torch.save(model.state_dict(), os.path.join(train_config['task_name'],
                                                        f"ddpm_ckpt_{epoch_idx}.pth"))
