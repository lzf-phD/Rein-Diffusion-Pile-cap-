...
def train(args):
    ...
    for epoch_idx in range(num_epochs):
        losses = []
        for im,txt,hint in tqdm(data_loader):
            optimizer.zero_grad()
            im = im.float().to(device)
            txt = txt.float().to(device)
            ...
            noise_pred = model(noisy_im,txt, t, hint)
        ...
        print('Finished epoch:{} | Loss : {:.4f}'.format(
            ...
            np.mean(losses)))
        lr_scheduler.step()
        if epoch_idx % 5 == 0 or epoch_idx + 1 == num_epochs:  # 每5轮保存一次
            torch.save(model.state_dict(), os.path.join(train_config['task_name'],
                                                        f"ddpm_controlnet_ckpt_{epoch_idx}.pth")) #每轮保存试一下不知道效果
