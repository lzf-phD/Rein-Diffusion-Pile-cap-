    ...
    def forward(self, x,context, t, hint):
        ...
        with torch.no_grad():
            ...
            for idx, down in enumerate(self.trained_unet.downs):
                ...
                train_unet_out = down(train_unet_out,context, trained_unet_t_emb)
        ...
        for idx, down in enumerate(self.control_unet.downs):
            ...
            control_unet_out = down(control_unet_out,context,control_unet_t_emb)

        for idx in range(len(self.control_unet.mids)):
            # Get midblock output of controlnets copy of unet
            control_unet_out = self.control_unet.mids[idx](control_unet_out,context, control_unet_t_emb)

            # Get midblock output of trained unet
            train_unet_out = self.trained_unet.mids[idx](train_unet_out,context, trained_unet_t_emb)
