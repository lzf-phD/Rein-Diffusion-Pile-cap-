...
class ControlNet(nn.Module):
    ...
    def __init__(self, model_config,
        ...
        self.trained_unet = Unet(model_config) #Unet模型
        ...
        self.control_copy_unet_hint_block = nn.Sequential(
            nn.Conv2d(model_config['hint_channels'],  #hint_channels=3
                      64,
                      kernel_size=3,
                      padding=(1, 1)),
            nn.SiLU(),
        ...
        self.control_copy_unet_mid_zero_convs = nn.ModuleList([
            make_zero_module(nn.Conv2d(self.trained_unet.mid_channels[i],
                                       ...
                                       kernel_size=1,
                                       padding=0))
            for i in range(1, len(self.trained_unet.mid_channels)) #self.trained_unet为原本Unet模型，把其中模块取过来重新组装
        ])
    ...
    def get_params(self):
        ...
        # them as well
        if not self.model_locked: #False
    ...
    def forward(self, x,context, t, hint):
        ...
        for idx, down in enumerate(self.control_copy_unet.downs): #3个module
            # Save the control nets copy output after passing it through zero conv layers
            control_copy_unet_down_outs.append(
                self.control_copy_unet_down_zero_convs[idx](control_copy_unet_out) #3个module
            )
            control_copy_unet_out = down(control_copy_unet_out,context, control_copy_unet_t_emb)

        for idx in range(len(self.control_copy_unet.mids)): #2module
            # Get midblock output of controlnets copy of unet
            control_copy_unet_out = self.control_copy_unet.mids[idx](
                control_copy_unet_out,context,
                control_copy_unet_t_emb
            )#[256,7,7]

            # Get midblock output of trained unet
            train_unet_out = self.trained_unet.mids[idx](train_unet_out,context, trained_unet_t_emb)#[256,7,7]

            # Add midblock output of controlnets copy of unet to that of trained unet
            # but after passing them through zero conv layers
            train_unet_out += self.control_copy_unet_mid_zero_convs[idx](control_copy_unet_out) #hint+x
        ...
        for up in self.trained_unet.ups:
            ...
            train_unet_out = up(train_unet_out,context,
                                control_copy_unet_down_out + trained_unet_down_out,
                                trained_unet_t_emb)

        # Call output layers of trained unet
        train_unet_out = self.trained_unet.norm_out(train_unet_out) #[16，28，28]
        train_unet_out = nn.SiLU()(train_unet_out)
        train_unet_out = self.trained_unet.conv_out(train_unet_out) #[16，28，28]->[1,28,28]
        # out B x C x H x W
